...
type Scheduler struct {
	...
	AppMap map[string]map[string][]string
}
...
func (sched *Scheduler) Run(ctx context.Context) {
	...
	sched.AppMap = make(map[string]map[string][]string)
	// sched.AppMap["Spark"] = make(map[string][]string)
...
func (sched *Scheduler) scheduleOne(ctx context.Context) {
	// fmt.Print("Calling Pending Pods", sched.SchedulingQueue.PendingPods())
	// klog.Infof("Entering into the ScheduleOne!!!")
	...
	// Node["1"] = append(Node["1"], "exec1")
	// Node["1"] = append(Node["1"], "exec2")

	// sched.AppMap["Spark"] = Node
	...
	defer cancel()
	scheduleResult, err := sched.Algorithm.Schedule(schedulingCycleCtx, fwk, state, pod, sched.AppMap)
	...
	// klog.Infof("Attempting to schedule pod on to the following node: %v", scheduleResult.SuggestedHost)

	// scheduleResult.SuggestedHost = "node-1"
	...
	sched.AppMap["Spark"][scheduleResult.SuggestedHost] = append(sched.AppMap["Spark"][scheduleResult.SuggestedHost], pod.Name)

	// sched.AppMap["Spark"] = Nodes

	// for k, v := range sched.AppMap {
	// 	fmt.Println(k, v)
	// }
