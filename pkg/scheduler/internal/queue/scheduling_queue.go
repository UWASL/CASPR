...
func (p *PriorityQueue) Add(pod *v1.Pod) error {
	...
	// klog.Infof("Hi I am in the Scheduling Queue Adding:", pod.Name)
	
	p.lock.Lock()
	defer p.lock.Unlock()
	pInfo := p.newQueuedPodInfo(pod)

	// klog.Infof("P Info", pInfo)
	...
	// klog.Infof("Printing P.ActiveQ.Pods:", p.activeQ.List())
	// fmt.Println("Printing P.ActiveQ.Pods:", p.activeQ.List())
	// fmt.Print("Current Pod Queue:", *plist )
...
func (p *PriorityQueue) Update(oldPod, newPod *v1.Pod) error {
	...
	fmt.Print("Reaching the Update Call in the Scheduling_Queue")
...
func (p *PriorityQueue) movePodsToActiveOrBackoffQueue(podInfoList []*framework.QueuedPodInfo, event string) {
	for _, pInfo := range podInfoList {
		...
		if p.isPodBackingoff(pInfo) {
			...
			fmt.Print("Line 537 in movePodsToActiveOrBackoffQueue")
			if err := p.activeQ.Add(pInfo); err != nil {
...
func (p *PriorityQueue) PendingPods() []*v1.Pod {
	...
	fmt.Println("Pending Pods::", result)
	klog.Infof("Pending Pods in KLOG INFO", result )
