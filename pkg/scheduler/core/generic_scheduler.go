...
type ScheduleAlgorithm interface {
	Schedule(context.Context, framework.Framework, *framework.CycleState, *v1.Pod, map[string]map[string][]string) (scheduleResult ScheduleResult, err error)
...
func (g *genericScheduler) Schedule(ctx context.Context, fwk framework.Framework, state *framework.CycleState, pod *v1.Pod, AppMap map[string]map[string][]string) (result ScheduleResult, err error) {
	...
	host, err := g.selectConnectedHost(priorityList, pod.Name, AppMap)
...
func findedges(d [5][5]int) [5]int {
...
// nodeScoreList gives me the list of nodes to choose from.
func (g *genericScheduler) selectConnectedHost(nodeScoreList framework.NodeScoreList, podname string, AppMap map[string]map[string][]string) (string, error) {
	if len(nodeScoreList) == 0 {
		return "", fmt.Errorf("empty priorityList")
	}

	m := make(map[string]int) //map nodes to index
	m["node-0"] = 0
	m["node-1"] = 1
	m["node-2"] = 2
	m["node-3"] = 3
	...
	// 	fmt.Println("Printing AppMap")
	// 	fmt.Println(k, v)
	// }

	// fmt.Println("NodeScoreList:", nodeScoreList)

	dat, err := ioutil.ReadFile("/storage/hello/hello.txt")
	...
	var str string
	str = string(dat)
	// fmt.Print("Hey check this out: ")
	// fmt.Print(str)

	adjacencyMatrix := [5][5]int{}
	...
	for i := 0; i < 5; i++ {
		for j := 0; j < 5; j++ {
			adjacencyMatrix[i][j] = res[index]
			index++
		}
	}

	// fmt.Println("Adjacency Matrix:", adjacencyMatrix)

	edges := findedges(adjacencyMatrix)
	...
	selectedEdges := edges[m[selected]]

	if strings.Contains(podname, "spark") {

		// fmt.Println("Okay Im choosing the Node for Spark")

		// fmt.Println("Here is the list of nodes that I have", nodeScoreList)

		// fmt.Println("I have initially Selected", maxScore, selected, selectedEdges)

		cntOfMaxScore := 1
		for _, ns := range nodeScoreList[1:] {
			// fmt.Println("Now I am comparing the selected to", ns.Name, ns.Score)
			nsEdges := edges[m[ns.Name]]
			// fmt.Println("Comparing the edges of ", ns.Name, "With the edges of", selected)
			if nsEdges > selectedEdges {	
				maxScore = ns.Score
				selected = ns.Name
				selectedEdges = nsEdges
				cntOfMaxScore = 1					
				// if ns.Score > maxScore { //Only choosing the next best node if it mores connected than the current node
				// // Have to check if nifty reroutes is detected and then using a different algorithm
				// 	maxScore = ns.Score
				// 	selected = ns.Name
				// 	cntOfMaxScore = 1
				// }
			} else if nsEdges == selectedEdges {
		...
		_, ok := AppMap["Spark"]["node-1"]
		if ok {
			prevNodes[m["node-1"]]++
		}

		_, ok = AppMap["Spark"]["node-2"]
		if ok {
			prevNodes[m["node-2"]]++
		}

		_, ok = AppMap["Spark"]["node-3"]
		if ok {
			prevNodes[m["node-3"]]++
		}

		_, ok = AppMap["Spark"]["node-4"]
		if ok {
			prevNodes[m["node-4"]]++
		}

		for _, ns := range nodeScoreList {
			if prevNodes[m[ns.Name]] == 1 {
				// fmt.Println("Hi i am starting with Node", ns.Name)
				selected = ns.Name
				maxScore = ns.Score
				break
			}
		...
		// fmt.Println("Printing PrevNodes", prevNodes)
		// fmt.Println("Node Score List is also as follows: ", nodeScoreList)
		...
		for _, ns := range nodeScoreList {
			...
			if ns.Score > maxScore {
				// fmt.Println("Hello Now I am comparing it to", ns.Name)
				if adjacencyMatrix[m[ns.Name]][m[selected]] == 1 {
						// fmt.Println("I initially chose", ns.Name)
						// maxScore = ns.Score
						// selected = ns.Name
						cntOfMaxScore = 1
						// fmt.Println("Check if its connected to everyone")
						//check if it is connected to everyone else 
						for i,j:= range prevNodes{
							// fmt.Println("Checking if its connected to ", i)
							if j != 1 {
								// fmt.Println("I don't need to check its not being used")
								continue
							}
							if adjacencyMatrix[i][m[ns.Name]] == 1{
								cnctd = true
							} else {
								cnctd = false
								break
